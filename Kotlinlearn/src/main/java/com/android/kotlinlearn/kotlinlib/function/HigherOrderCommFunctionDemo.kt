package com.android.kotlinlearn.kotlinlib.function
...
/**
 * 测试T.run{}函数，和with(){}不同于with有两个参数和this指向的引用不同，with中的this指向的是它的第一个参数，而run中的this指向的是对象实例
 */
 ...
 *  T.let 可返回了一个不同类型的值，但是 T.also 返回了 T 自身类型
 */
fun testLet() {
    "kotlin".let {
