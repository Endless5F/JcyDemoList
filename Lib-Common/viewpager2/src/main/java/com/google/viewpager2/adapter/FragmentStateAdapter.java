...
import com.google.viewpager2.widget.ViewPager2;
...
public abstract class FragmentStateAdapter extends
    ...
    private boolean isFragmentViewBound(long itemId) {
        if (mItemIdToViewHolder.indexOfKey(itemId) >= 0) {
            return true;
        }
    ...
    private void ensureFragment(int position) {
        ...
        if (!(mFragments.indexOfKey(itemId) >= 0)) {
...
//                    .setMaxLifecycle(fragment, STARTED)
    ...
    public final void restoreState(@NonNull Parcelable savedState) {
        if (mSavedStates.size() != 0 || mFragments.size() != 0) {
    ...
    class FragmentMaxLifecycleEnforcer {
        ...
        void updateFragmentMaxLifecycle(boolean dataSetChanged) {
            ...
            if (mFragments.size() == 0 || getItemCount() == 0) {
